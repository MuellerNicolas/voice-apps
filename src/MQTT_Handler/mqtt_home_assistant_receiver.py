...
class MQTTHomeAssistantReceiver(mqtt.Client):
    def __init__(self, broker):
        ...
        path = os.path.join(os.path.dirname(__file__), 'mqtt_home_assistant_settings.json')
        with open(path) as f:
            mqtt_settings = json.load(f)
        path = os.path.join(os.path.dirname(__file__), 'MQTT_Home_Assistant_Authorization.json')
        with open(path) as f:
    ...
    def _broker_notify_alarm_info(self, mosq, obj, msg):
        ...
        alarm_info = {}
        alarm_info["hour"], alarm_info["minute"], alarm_info["second"] = map(int, response["time"].split(':'))
        ...
        get_logger(__name__).info(f'received & published the following alarm_infos={alarm_info}, alarm_song={alarm_song}')
    ...
    def _run(self):
        ...
        try:
            ...
            self.message_callback_add('home-assistant/alarm-clock/nicolas', self._broker_notify_alarm_info)
