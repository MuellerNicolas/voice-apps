...
def buzz(frequency, length):	 #create the function "buzz" and feed it the pitch and duration)
	...
	for i in range(numCycles):		#start a loop from 0 to the variable "cycles" calculated above
		gpio.setDigital(buzzer_pin, 'ON')	 #set buzzer_pin to high
		time.sleep(delayValue)		#wait with buzzer_pin high
		gpio.setDigital(buzzer_pin, 'OFF')		#set buzzer_pin to low
		time.sleep(delayValue)		#wait with buzzer_pin low
	


def setup():
	gpio.setFunction(buzzer_pin, 'DIGITAL')
	gpio.setMode(buzzer_pin, 'output')
	
def close():
	gpio.setDigital(buzzer_pin,"OFF")				# Set the pin to off
...
if __name__ == '__main__':		# Program start from here
	try:
		...
		close()				# When 'Ctrl+C' is pressed and in any other case, the child program close() will be  executed.